...
echo Starting Celery (Background Worker with Parallel Concurrency)...
start "Celery Worker" cmd /k "cd Backend && call venv\Scripts\activate && celery -A app.celery_app worker --pool=threads --concurrency=8 -l info"
